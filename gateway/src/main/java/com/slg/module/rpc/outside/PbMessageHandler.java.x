...
//                            out.release();
            ...
            } else if (userId != null) {//转
                Integer connectionId = serverChannelManage.getChannelIdByUser(userId, protocolId);
                if (connectionId == null) {//建立,链接
                    connectionId = serverChannelManage.allocationChannelToUserId(protocolId, userId);
                }
     ...
     * @param msg           消息
     */
//    private void forwardToTargetServer2(ChannelHandlerContext clientChannel, ByteBufferMessage msg, long userId, ServerConfig serverConfig) {
//        //异步建立链接，刚刚开始会为null
...
//        }
//    }
    private void forwardToTargetServer(ChannelHandlerContext clientChannel, Channel insideChannel, ByteBufferMessage msg, long userId, ServerConfig serverConfig) {
        //进行转发到目标服务器
        if (insideChannel != null && insideChannel.isActive()) {
            forward(insideChannel, clientChannel, msg, userId, ErrorCodeConstants.GATE_FORWARDING_FAILED, serverConfig);
        } else {
            failedNotificationClient(clientChannel, msg, ErrorCodeConstants.GATE_FORWARDING_FAILED);
        }
    }
    ...
    //发送消息
    public void forward2(Channel serverChannel, ChannelHandlerContext clientChannel, ByteBufferMessage msg, long userId, int errorCode, Integer serverId) {
        ...
        channelFuture.addListener((ChannelFutureListener) future -> {
            if (future.isSuccess()) {
                ...
                serverChannelManage.removeServerChanel(serverId);
                //直接告诉客户端，返回错误码
